...
def main(robot):
    ...
    def input_handle(message, session):
        ...
        if uuid:
            ...
            message = mail_channel.sudo(request_uid).with_context(mail_create_nosubscribe=True).message_post(
                ...
                subtype='mail.mt_comment', content_subtype='plaintext', weixin_id=openid)
    ...
    def input_handle(message, session):
        ...
        addons_path = http.addons_manifest['wx_tools']['addons_path']
        ...
        img_path_name = os.path.join(addons_path, 'wx_tools', 'static', 'src', 'img', filename)
        ...
        if uuid:
            web_filename = os.path.join('/wx_tools', 'static', 'src', 'img', filename)
